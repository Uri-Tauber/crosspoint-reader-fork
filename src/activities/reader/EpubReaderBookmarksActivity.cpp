...
void EpubReaderBookmarksActivity::render(RenderLock&&) {
  ...
  const auto getBookmarkSubtitle = [this](int index) {
    ...
    std::string text = std::to_string((int)(std::clamp(bookmark.percentage, 0.0f, 1.0f) * 100.0f + 0.5f));
    text.append("% - ").append(std::to_string(bookmark.computedChapterProgress + 1));
    text.append("/").append(std::to_string(bookmark.computedChapterPageCount));
    text.append(" - ").append(tocTitle);
    return text;
  };
