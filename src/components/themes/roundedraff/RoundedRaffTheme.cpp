...
void RoundedRaffTheme::drawOptionPopup(const GfxRenderer& renderer, const char* title,
                                       const std::vector<std::string>& options, int selectedIndex) const {
  const auto pageWidth = renderer.getScreenWidth();
  const auto pageHeight = renderer.getScreenHeight();
  const auto height10 = renderer.getLineHeight(kTitleFontId);
  const auto height12 = renderer.getLineHeight(kTitleFontId);  // Using kTitleFontId for title as well

  constexpr int itemSpacing = kSelectableRowGap;
  constexpr int innerPadding = 24;
  constexpr int selectionHPadding = kInteractiveInsetX;
  constexpr int selectionVPadding = 10;

  const int rowHeight = height10 + selectionVPadding * 2;

  int maxTextWidth = renderer.getTextWidth(kTitleFontId, title, EpdFontFamily::BOLD);
  for (const auto& opt : options) {
    int w = renderer.getTextWidth(kTitleFontId, opt.c_str(), EpdFontFamily::BOLD);
  ...
  const int dialogW = std::min((maxTextWidth + innerPadding * 2 + selectionHPadding * 2) * 12 / 10,
                               pageWidth - RoundedRaffMetrics::values.contentSidePadding * 2);
  const int contentHeight = height12 + 16 + listHeight;
  const int dialogH = contentHeight + innerPadding * 2;
  const int dialogX = (pageWidth - dialogW) / 2;
  const int dialogY = (pageHeight - dialogH) / 2;

  const int frameThickness = RoundedRaffMetrics::values.popupFrameThickness;
  const int frameRadius = RoundedRaffMetrics::values.popupCornerRadius;
  ...
  for (int i = 0; i < optionCount; i++) {
    ...
    renderer.fillRoundedRect(itemRectX, itemY, itemRectW, rowHeight, kMenuRadius,
                             selected ? Color::Black : Color::White);

    const int textH = renderer.getLineHeight(kTitleFontId);
    const int textW = renderer.getTextWidth(kTitleFontId, labelText, EpdFontFamily::BOLD);
    const int textY = itemY + (rowHeight - textH) / 2;
    const int textX = itemRectX + (itemRectW - textW) / 2;

    renderer.drawText(kTitleFontId, textX, textY, labelText, !selected, EpdFontFamily::BOLD);
  }
}
