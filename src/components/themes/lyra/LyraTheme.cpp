...
void LyraTheme::drawOptionPopup(const GfxRenderer& renderer, const char* title, const std::vector<std::string>& options,
                                int selectedIndex) const {
  const auto pageWidth = renderer.getScreenWidth();
  const auto pageHeight = renderer.getScreenHeight();
  const auto height10 = renderer.getLineHeight(UI_12_FONT_ID);
  const auto height12 = renderer.getLineHeight(UI_12_FONT_ID);

  constexpr int itemSpacing = 8;
  constexpr int innerPadding = 20;
  constexpr int selectionHPadding = 16;
  constexpr int selectionVPadding = 12;

  int maxTextWidth = renderer.getTextWidth(UI_12_FONT_ID, title, EpdFontFamily::BOLD);
  for (const auto& opt : options) {
    int w = renderer.getTextWidth(UI_12_FONT_ID, opt.c_str());
  ...
  const int dialogW = std::min((maxTextWidth + innerPadding * 2 + selectionHPadding * 2) * 12 / 10,
                               pageWidth - LyraMetrics::values.contentSidePadding * 2);
  const int contentHeight = height12 + 16 + listHeight;
  const int dialogH = contentHeight + innerPadding * 2;
  const int dialogX = (pageWidth - dialogW) / 2;
  const int dialogY = (pageHeight - dialogH) / 2;

  const int frameThickness = LyraMetrics::values.popupFrameThickness;
  const int frameRadius = LyraMetrics::values.popupCornerRadius;
  ...
  for (int i = 0; i < optionCount; i++) {
    ...
    if (selected) {
      renderer.fillRoundedRect(itemRectX, itemY, itemRectW, rowHeight, cornerRadius, Color::LightGray);
    }

    const int textH = renderer.getLineHeight(UI_12_FONT_ID);
    const int textW = renderer.getTextWidth(UI_12_FONT_ID, labelText);
    const int textY = itemY + (rowHeight - textH) / 2;
    const int textX = itemRectX + (itemRectW - textW) / 2;

    renderer.drawText(UI_12_FONT_ID, textX, textY, labelText, true);
  }
}
