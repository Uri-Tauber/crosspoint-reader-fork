...
void LyraTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle) const {
  ...
  if (title) {
    ...
    drawTextStart(renderer, UI_12_FONT_ID, leftX, rightX, rect.y + LyraMetrics::values.batteryBarHeight + 3,
  ...
  if (subtitle) {
    ...
    const int leftX = rect.x + LyraMetrics::values.contentSidePadding;
    const int rightX = rect.x + rect.width - LyraMetrics::values.contentSidePadding;
    drawTextEnd(renderer, SMALL_FONT_ID, leftX, rightX, rect.y + 50, truncatedSubtitle.c_str(), true);
  }
}
...
void LyraTheme::drawSubHeader(const GfxRenderer& renderer, Rect rect, const char* label, const char* rightLabel) const {
  ...
  drawTextStart(renderer, UI_10_FONT_ID, leftX, rightX, rect.y + 6, truncatedLabel.c_str(), true,
...
void LyraTheme::drawList(const GfxRenderer& renderer, Rect rect, int itemCount, int selectedIndex,
  ...
  for (int i = pageStartIndex; i < itemCount && i < pageStartIndex + pageItems; i++) {
    ...
    drawTextStart(renderer, UI_10_FONT_ID, textLeft, textRight, itemY + 7, item.c_str(), true);
    ...
    if (rowSubtitle != nullptr) {
      ...
      drawTextStart(renderer, SMALL_FONT_ID, textLeft, textRight, itemY + 30, subtitle.c_str(), true);
    }
    ...
    if (!valueText.empty()) {
      ...
      drawTextEnd(renderer, UI_10_FONT_ID, rect.x + LyraMetrics::values.contentSidePadding,
...
void LyraTheme::drawEmptyRecents(const GfxRenderer& renderer, const Rect rect) const {
  ...
  drawTextStart(renderer, UI_12_FONT_ID, rect.x + padding, rect.x + rect.width - padding,
                rect.y + rect.height / 2 - renderer.getLineHeight(UI_12_FONT_ID) - 2, tr(STR_NO_OPEN_BOOK), true,
                EpdFontFamily::BOLD);
  drawTextStart(renderer, UI_10_FONT_ID, rect.x + padding, rect.x + rect.width - padding, rect.y + rect.height / 2 + 2,
...
void LyraTheme::drawButtonMenu(GfxRenderer& renderer, Rect rect, int buttonCount, int selectedIndex,
  ...
  for (int i = 0; i < buttonCount; ++i) {
    ...
    drawTextStart(renderer, UI_12_FONT_ID, textLeft, textRight, textY, label, true);
  }
}
